...
function legacyParse(input, parseQueryString, whatwgError, slashesDenoteHost) {
  const parsed = import_node_url.default.parse(input, parseQueryString, slashesDenoteHost);
  ...
  if (authority && (authority[1] || parsed.hostname !== null)) {
    ...
    if (written.charAt(0) === "[" && !import_node_net.default.isIPv6(written.slice(1, written.indexOf("]")))) {
      throw whatwgError;
    }
...
var resolve = (from, to) => {
  try {
    ...
    return import_node_url.default.resolve(from, to);
  }
};
  ...
  set(cookieStr, url) {
    ...
    if (cookie.domain) {
      ...
      if (
        ...
        import_node_net2.default.isIP(urlparts.hostname) || // prefix domains with dot to be sure that partial matches are not used
...
function nmfetch(url, options) {
  ...
  if (parsed.protocol === "https:" && parsed.hostname && parsed.hostname !== reqOptions.host && !import_node_net3.default.isIP(parsed.hostname) && !reqOptions.servername) {
  ...
  req.on("response", (res) => {
    ...
    switch (res.headers["content-encoding"]) {
      case "gzip":
      case "deflate":
        inflate = import_node_zlib.default.createUnzip();
        break;
    }
...
try {
  networkInterfaces = import_node_os.default.networkInterfaces();
} catch (_err) {
}
...
var resolve2 = (family, hostname, options, callback) => {
  ...
  const dnsResolver = import_node_dns.default.Resolver ? new import_node_dns.default.Resolver(options) : import_node_dns.default;
  dnsResolver["resolve" + family](hostname, (err, addresses) => {
    if (err) {
      switch (err.code) {
        case import_node_dns.default.NODATA:
        case import_node_dns.default.NOTFOUND:
        case import_node_dns.default.NOTIMP:
        case import_node_dns.default.SERVFAIL:
        case import_node_dns.default.CONNREFUSED:
        case import_node_dns.default.REFUSED:
        case "EAI_AGAIN":
          return callback(null, []);
      }
      return callback(err);
...
var resolveHostname = (options, callback) => {
  ...
  if (!options.host || import_node_net4.default.isIP(options.host)) {
    const value = {
...
function createDefaultLogger(levels) {
  ...
  const print = (level, entry, message, ...args) => {
    ...
    message = import_node_util.default.format(message, ...args);
    message.split(/\r?\n/).forEach((line) => {
...
function detectMimeType(filename) {
  ...
  const parsed = import_node_path.default.parse(filename);
...
var Encoder = class extends import_node_stream2.Transform {
  constructor(options) {
    super();
    this.options = options || {};
...
var Encoder2 = class extends import_node_stream3.Transform {
  constructor(options) {
    super();
    this.options = options || {};
    if (this.options.lineLength !== false) {
...
function normalizeDomain(domain, toUnicode2) {
  const mapper = toUnicode2 ? import_node_url2.default.domainToUnicode : import_node_url2.default.domainToASCII;
...
var MimeNode = class _MimeNode {
  constructor(contentType, options) {
    this.nodeCounter = 0;
    options = options || {};
    this.baseBoundary = options.baseBoundary || import_node_crypto.default.randomBytes(8).toString("hex");
  ...
  _getStream(content) {
    ...
    if (content._resolvedValue) {
      contentStream = new import_node_stream7.PassThrough();
      setImmediate(() => {
        try {
    ...
    if (content && typeof content.path === "string" && !content.href) {
      ...
      return import_node_fs2.default.createReadStream(content.path);
    }
  ...
  _generateMessageId() {
    return "<" + [2, 2, 2, 6].reduce(
      ...
      (prev, len) => prev + "-" + import_node_crypto.default.randomBytes(len).toString("hex"),
      import_node_crypto.default.randomBytes(4).toString("hex")
...
var RelaxedBody = class extends import_node_stream9.Transform {
  constructor(options) {
    super();
    options = options || {};
    this.bodyHash = import_node_crypto2.default.createHash(options.hashAlgo || "sha256");
    this.byteLength = 0;
...
function sign(headers, hashAlgo, bodyHash, options) {
  ...
  const signer = import_node_crypto3.default.createSign(("rsa-" + hashAlgo).toUpperCase());
...
var DKIMSigner = class {
  constructor(options, keys, input, output) {
    ...
    this.chunklen = 0;
    this.readPos = 0;
    this.cachePath = this.cacheDir ? import_node_path2.default.join(this.cacheDir, "message." + Date.now() + "-" + import_node_crypto4.default.randomBytes(14).toString("hex")) : false;
    this.cache = false;
    this.headers = false;
    this.bodyHash = false;
    this.parser = false;
  ...
  cleanup() {
    ...
    import_node_fs3.default.unlink(this.cachePath, () => false);
  }
  createReadCache() {
    this.cache = import_node_fs3.default.createReadStream(this.cachePath);
  ...
  createWriteCache() {
    ...
    this.cache = import_node_fs3.default.createWriteStream(this.cachePath);
    this.cache.once("error", (err) => {
      this.cleanup();
...
var DKIM = class {
  ...
  sign(input, extraOptions) {
    const output = new import_node_stream10.PassThrough();
    let inputStream = input;
    let writeValue = false;
    if (Buffer.isBuffer(input)) {
      writeValue = input;
      inputStream = new import_node_stream10.PassThrough();
...
function httpProxyClient(proxyUrl, destinationPort, destinationHost, tlsOptions, callback) {
  ...
  if (proxy.protocol === "https:") {
    ...
    connect = import_node_tls.default.connect.bind(import_node_tls.default);
  } else {
    connect = import_node_net5.default.connect.bind(import_node_net5.default);
  }
  let socket;
  let finished = false;
...
var Mail = class extends import_node_events.EventEmitter {
    ...
    this.getSocket = (options, callback) => {
      ...
      switch (protocol) {
        ...
        case "socks4a": {
          ...
          if (import_node_net6.default.isIP(proxy.hostname)) {
            return connect(proxy.hostname);
          }
          return import_node_dns2.default.resolve(proxy.hostname, (err2, address) => {
            if (err2) {
              return callback(err2);
            }
  ...
  _convertDataImages(mail, callback) {
    ...
    mail.resolveContent(mail.data, "html", { disableFileAccess: mail.data.disableFileAccess, disableUrlAccess: mail.data.disableUrlAccess }, (err, html) => {
      ...
      html = (html || "").toString().replace(/(<img\b[^<>]{0,1024} src\s{0,20}=[\s"']{0,20})(data:([^;]+);[^"'>\s]+)/gi, (match, prefix, dataUri, mimeType) => {
        const cid = import_node_crypto5.default.randomBytes(10).toString("hex") + "@localhost";
...
var SMTPConnection = class extends import_node_events2.EventEmitter {
  constructor(options) {
    super(options);
    this.id = import_node_crypto6.default.randomBytes(8).toString("base64").replace(/\W/g, "");
    this.stage = "init";
    this.options = options || {};
    ...
    this.servername = this.options.servername ? this.options.servername : !import_node_net7.default.isIP(this.host) ? this.host : false;
  ...
  _connectToHost(opts, secure) {
    ...
    const connectFn = secure ? import_node_tls2.default.connect : import_node_net7.default.connect;
    try {
  ...
  send(envelope, message, done) {
    ...
    this._setEnvelope(envelope, (err, info) => {
      if (err) {
        const stream2 = new import_node_stream12.PassThrough();
  ...
  _upgradeConnection(callback) {
    ...
    this.upgrading = true;
    try {
      this._socket = import_node_tls2.default.connect(opts, () => {
        this.secure = true;
  ...
  _createSendStream(callback) {
    ...
    if (this.options.debug) {
      const logStream = new import_node_stream12.PassThrough();
  ...
  _actionAUTH_CRAM_MD5(str, callback) {
    ...
    const hmacMD5 = import_node_crypto6.default.createHmac("md5", this._auth.credentials.pass);
  ...
  _getHostname() {
    let defaultHostname;
    try {
      defaultHostname = import_node_os2.default.hostname() || "";
    } catch (_err) {
...
var XOAuth2 = class extends import_node_stream13.Stream {
  ...
  jwtSignRS256(payload) {
    ...
    const signature = import_node_crypto7.default.createSign("RSA-SHA256").update(signedPayload).sign(this.options.privateKey);
...
var SESTransport = class extends import_node_events6.default {
  constructor(options) {
    super();
    options = options || {};
    this.options = options;
...
function normalizeContactPayload(body) {
  const type = body.type === "kiduart" || body.type === "demo" ? String(body.type) : "services";
  const name2 = String(body.name || body.contactName || "").trim();
  ...
  return { type, name: name2, email, organization, phone, scope };
}
...
async function handleContact(body) {
  ...
  const { type, name: name2, email, organization, phone, scope } = normalizeContactPayload(body || {});
  if (!name2 || !email || !organization) {
    return {
      ok: false,
      status: 400,
  ...
  const inbox = process.env.MAIL_TO || (type === "kiduart" || type === "demo" ? "support@kiduart.com" : "contact@trevyk.com");
  const fromAddress = process.env.SMTP_USER;
  const subjectPrefix = type === "kiduart" || type === "demo" ? "[Kiduart demo]" : "[Trevyk contact]";
  const subject = `${subjectPrefix} ${organization}  ${name2}`;
  const textBody = [
    `Type: ${type}`,
    `Name: ${name2}`,
    `Email: ${email}`,
  ...
  const htmlBody = `
      <div style="font-family:Arial,sans-serif;line-height:1.5;color:#241428">
        ...
        <p><strong>Name:</strong> ${escapeHtml(name2)}</p>
        ...
        <p style="color:#666;font-size:12px">Submitted ${escapeHtml((/* @__PURE__ */ new Date()).toISOString())}</p>
      </div>
    `;
  try {
    ...
    try {
      await transport.sendMail({
        ...
        text: [
          ...
          "",
          type === "kiduart" || type === "demo" ? "You can also book a product demo anytime at https://kiduart.com" : "Meanwhile you can explore https://trevyk.in",
          "",
          " Trevyk Technologies"
        ].join("\n")
      });
    ...
    console.error("Contact mail error:", error2);
    const raw = error2 instanceof Error ? error2.message : "Could not send your message right now. Please email contact@trevyk.com.";
    const isBadCreds = /Invalid login|BadCredentials|535-5\.7\.8/i.test(raw) || /Username and Password not accepted/i.test(raw);
    return {
      ok: false,
      status: 500,
      error: isBadCreds ? "Gmail rejected SMTP login (535 BadCredentials). Create a new App Password for tech.trevyk@gmail.com at https://myaccount.google.com/apppasswords (2-Step Verification must be on), put it in SMTP_PASS, restart the server, and try again. Do not use your normal Gmail password." : raw
    };
  }
}
